    ...
    this._zone.runOutsideAngular(() => {
      ...
      const mousedrag$ = mousedown$.pipe(
        switchMap((event: MouseEvent) => {
          if (event.toElement["type"]
            || event.toElement.tagName.toUpperCase() === "INPUT"
            || event.toElement.tagName.toUpperCase() === "IMG"
            || event.toElement.tagName.toUpperCase() === "SVG"
            || (event.toElement && typeof event.toElement.className === "string" && (event.toElement.className.indexOf("mat-button") > -1
            || event.toElement.className.indexOf("mat-icon") > -1))) {
